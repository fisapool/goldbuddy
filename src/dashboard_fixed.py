...
# Cache currency rates for 5 minutes to avoid too many API calls
@lru_cache(maxsize=1)
def get_cached_rates(cache_key):
    """Get currency rates with a 5-minute cache"""
    c = CurrencyRates()
    rates = {
        "USD": 1.0,
        "MYR": c.get_rate("USD", "MYR"),
        "EUR": c.get_rate("USD", "EUR"),
        "GBP": c.get_rate("USD", "GBP"),
        "JPY": c.get_rate("USD", "JPY"),
        "SGD": c.get_rate("USD", "SGD"),
        "AUD": c.get_rate("USD", "AUD")
    }
    return rates

def get_current_rates():
    """Get current rates with cache invalidation every 5 minutes"""
    # Use timestamp rounded to 5 minutes as cache key
    cache_key = int(time.time() / 300)
    try:
        return get_cached_rates(cache_key)
    except Exception as e:
        st.warning("Could not fetch live rates. Using fallback rates.")
        return {
            "USD": 1.0,
            "MYR": 4.73,
            "EUR": 0.92,
...
def convert_currency(amount_usd, target_currency="USD"):
    """Convert USD amount to target currency using live rates"""
    rates = get_current_rates()
    return amount_usd * rates.get(target_currency, 1.0)

def format_currency(amount_usd, target_currency="USD"):
    """Format currency with appropriate symbol and conversion"""
    symbols = {
        "USD": "$", "MYR": "RM", "EUR": "€",
        "GBP": "£", "JPY": "¥", "SGD": "S$",
        "AUD": "A$"
    }
    converted_amount = convert_currency(amount_usd, target_currency)
    if target_currency == "JPY":
        return f"{symbols[target_currency]}{int(converted_amount):,}"
    return f"{symbols[target_currency]}{converted_amount:,.2f}"
...
if asset_type == "Stocks":
    ...
    tickers = st.sidebar.text_input("Enter Crypto Tickers (comma-separated)", "BTC,ETH,SOL").split(',')
...
for ticker in tickers:
    ...
    if three_commas and consensus:
        ...
        total_fee_percent = exchange_fee + 0.05  # Exchange fee + 3Commas fee (0.05%)
        ...
        with col_profit1:
            ...
            # Calculate fees
            entry_fee = position_size * (total_fee_percent / 100)
            ...
            best_case_fees = best_case * (total_fee_percent / 100) + subscription_per_trade
            worst_case_fees = worst_case * (total_fee_percent / 100) + subscription_per_trade
            expected_case_fees = expected_case * (total_fee_percent / 100) + subscription_per_trade
            ...
            scenarios_df = pd.DataFrame({
                ...
                'Trading Fees': [
                    format_currency(best_case * (total_fee_percent / 100), currency),
                    format_currency(expected_case * (total_fee_percent / 100), currency),
                    format_currency(worst_case * (total_fee_percent / 100), currency)
                ],
                'Subscription Cost': [
        ...
        with col_profit2:
            ...
            **{selected_plan} Plan - {format_currency(plan_details['monthly_fee'], currency)}/month**
            ...
            st.info(f"""
            **Fee Structure:**
            - Exchange Fee: {exchange_fee}% per trade
            ...
            **Monthly Cost Projection:**
            ...
            fee_impact = -(total_fee_percent + (subscription_per_trade/position_size) * 100)
            ...
            elif estimated_monthly_trades > 20 or position_size > 200:
        ...
        with col2:
            ...
            try:
                ...
                st.warning("Could not fetch existing bots. Check your API permissions.")
